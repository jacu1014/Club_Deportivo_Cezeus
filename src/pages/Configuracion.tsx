...
const Configuracion: React.FC<ConfiguracionProps> = ({ user }) => {
  ...
  // FIX: ADMINISTRATIVO agregado a canManageClub para que también vea la galería
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select(
          ...
          'email, telefono, foto_url, estado, tipo_documento, numero_documento, ' +
          'fecha_nacimiento, genero, direccion, eps, grupo_sanguineo, factor_rh, categoria'
        )
  ...
  const handleDelete = async (id: string, nombre: string) => {
    if (!window.confirm(`¿Estás seguro de eliminar permanentemente a ${nombre}?`)) return;
    try {
      setLoadingStaff(true);
  ...
  const handleResetPass = async (email: string) => {
    if (!window.confirm(`¿Enviar enlace de recuperación a: ${email}?`)) return;
    try {
      // FIX: usa la Edge Function reset-password via Resend
      // supabase.auth.resetPasswordForEmail() no funciona sin SMTP configurado
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20 px-4">
      ...
      <div className="flex flex-wrap gap-4 border-b border-white/5 pb-4">
        ...
        {canSeeStaff    && <TabButton active={activeTab === 'staff'}    onClick={() => setActiveTab('staff')}    icon="badge"    label="Personal" />}
        {canManageClub  && <TabButton active={activeTab === 'club'}     onClick={() => setActiveTab('club')}     icon="stadium"  label="Gestión Club" />}
        {canSeeActivity && <TabButton active={activeTab === 'actividad'} onClick={() => setActiveTab('actividad')} icon="history" label="Actividad" />}
      ...
      <div className="grid grid-cols-1 gap-8 pt-4">
          ...
          <div className="space-y-4">
            <div className="flex justify-end">
              <button
                ...
                className="bg-primary hover:bg-white text-black px-6 py-3 rounded-xl font-black
        ...
        {activeTab === 'club' && canManageClub && (
          // FIX: se pasa `user` como prop para que ClubSection pueda
          // verificar el rol y mostrar el GaleriaManager correctamente
