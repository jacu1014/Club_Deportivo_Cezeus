...
// Helper de módulo: calcula promedio desde items_evaluados (promedio no existe como columna en BD)
const calcularPromedio = (ev) => {
  if (!ev?.items_evaluados?.length) return 0;
  const suma = ev.items_evaluados.reduce((acc, i) => acc + (i.calificacion ?? 0), 0);
  return suma / ev.items_evaluados.length;
};
...
const SeguimientoAlumno = ({
  ...
  const datosRadar = useMemo(() => {
    ...
    return itemsCiclo.map(itemConfig => {
      // El hook normaliza como items_evaluados (no items)
      const notaInicial = inicial?.items_evaluados?.find(i => i.item_id === itemConfig.id)?.calificacion ?? 0;
      const notaFinal   = final?.items_evaluados?.find(i => i.item_id === itemConfig.id)?.calificacion ?? 0;
  ...
  const mejoraGral = useMemo(() => {
    const promedioInicial = calcularPromedio(evaluaciones.find(e => e.tipo === 'INICIAL'));
    const promedioFinal   = calcularPromedio(evaluaciones.find(e => e.tipo === 'FINAL'));
    if (promedioInicial === 0) return 0;
    return (((promedioFinal - promedioInicial) / promedioInicial) * 100).toFixed(0);
  }, [evaluaciones]);
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="bg-[#0a0f18]/60 border border-white/5 p-8 rounded-[2.5rem]">
             ...
             <h4 className="text-4xl text-primary font-black italic">
               {(() => {
                 const p = calcularPromedio(evaluaciones.find(e => e.tipo === 'FINAL'));
                 return p > 0 ? p.toFixed(1) : 'N/A';
               })()}
             </h4>
          </div>
