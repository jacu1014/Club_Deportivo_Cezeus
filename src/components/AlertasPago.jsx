...
import { format, getDate, setDate, isAfter, parseISO, startOfDay, differenceInDays, addMonths, subMonths } from 'date-fns';
...
const calcularEstadoPago = (alumno, pagosMap) => {
  ...
  let fechaCorteActual = startOfDay(setDate(new Date(), diaCorte));

  // Determinar el periodo que debemos validar:
  // Si hoy >= día de corte, validamos este mes. Si no, validamos desde el corte del mes pasado.
  const periodoValido = (isAfter(hoy, fechaCorteActual) || hoy.getTime() === fechaCorteActual.getTime())
    ? fechaCorteActual
    : startOfDay(subMonths(fechaCorteActual, 1));

  const pagosAlumno = pagosMap[alumno.id] || [];
  
  const tienePago = pagosAlumno.some(p => {
    const fPago = startOfDay(parseISO(p.fecha_pago));
    return fPago.getTime() >= periodoValido.getTime();
  });

  // Lógica de "Próximo a vencer": si está al día pero el corte es en 3 días o menos
  const diasParaCorte = differenceInDays(fechaCorteActual, hoy);
  const proximoAVencer = !isAfter(hoy, fechaCorteActual) && diasParaCorte <= 3 && diasParaCorte >= 0;

  if (tienePago) {
    return { 
      estado: 'AL_DIA', 
      fechaCorte: periodoValido, 
      ultimoPago: pagosAlumno[0],
      proximoAVencer: false 
    };
  }

  return {
    estado: proximoAVencer ? 'AL_DIA' : 'PENDIENTE',
    fechaCorte: periodoValido,
...
export default function AlertasPago({ currentUser }) {
  ...
  const esAlumno = currentUser?.rol === 'AL_NUMNO';
  ...
  const fetchDatos = async () => {
    ...
    try {
      ...
      const hace60dias = subMonths(new Date(), 2);
  ...
  const filtrados = useMemo(() => {
    let base = filtro === 'TODOS' ? alumnosConEstado : filtro === 'PENDIENTE' ? pendientes : alDia;
    if (busqueda.trim()) {
      const q = busqueda.toLowerCase();
      base = base.filter(a => `${a.primer_nombre} ${a.primer_apellido}`.toLowerCase().includes(q));
    }
    return base;
  ...
  if (esAlumno) {
    ...
    if (loading || !miEstado) return null;
    const pendiente = miEstado.pago?.estado === 'PENDIENTE';
    const proximo = miEstado.pago?.proximoAVencer;

    return (
      <div className={`rounded-[2rem] border p-6 flex items-center gap-4
                       ${pendiente ? 'bg-rose-500/10 border-rose-500/20' : 
                         proximo ? 'bg-amber-500/10 border-amber-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
        <span className={`material-symbols-outlined text-3xl
                          ${pendiente ? 'text-rose-400' : proximo ? 'text-amber-400' : 'text-emerald-400'}`}>
          {pendiente ? 'warning' : proximo ? 'schedule' : 'check_circle'}
        </span>
        <div>
          <p className={`font-black text-[11px] uppercase tracking-widest
                         ${pendiente ? 'text-rose-400' : proximo ? 'text-amber-400' : 'text-emerald-400'}`}>
            {pendiente ? 'Pago pendiente' : proximo ? 'Próximo a vencer' : 'Pago al día'}
          </p>
          <p className="text-slate-500 text-[10px] font-bold mt-0.5">
            {pendiente
              ...
              : proximo 
              ? `Tu corte es el ${format(miEstado.pago.fechaCorte, "d 'de' MMMM", { locale: es })} (en pocos días)`
              : `Último pago registrado el ${miEstado.pago.ultimoPago
                  ? format(parseISO(miEstado.pago.ultimoPago.fecha_pago), "d 'de' MMMM", { locale: es })
                  : '—'}`}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="bg-[#0a0f18]/60 border border-white/5 rounded-[1.5rem] sm:rounded-[2.5rem] overflow-hidden">
      {/* Header */}
      <div className="px-4 py-5 sm:px-8 sm:py-6 border-b border-white/5 bg-white/[0.02] flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <span className="material-symbols-outlined text-primary">payments</span>
          <div>
            <p className="font-black text-[10px] sm:text-[11px] text-white uppercase tracking-widest leading-none">Estado de Pagos</p>
            <p className="text-slate-600 text-[8px] sm:text-[9px] font-bold uppercase tracking-widest mt-1">Mensualidades · {format(new Date(), "MMMM yyyy", { locale: es }).toUpperCase()}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-3 sm:gap-4 bg-white/[0.03] md:bg-transparent p-3 md:p-0 rounded-2xl border border-white/5 md:border-none">
          ...
          <button onClick={fetchDatos} className="ml-auto md:ml-0 p-2 bg-white/5 rounded-xl border border-white/10 hover:bg-primary/10 transition-all">
            <span className={`material-symbols-outlined text-slate-400 text-sm ${loading ? 'animate-spin' : ''}`}>refresh</span>
          </button>
        </div>
      </div>
      ...
      <div className="px-4 py-4 sm:px-8 border-b border-white/5 flex flex-col sm:flex-row gap-4 items-center">
        <div className="flex gap-2 overflow-x-auto w-full sm:w-auto pb-2 sm:pb-0 scrollbar-hide">
          {[{ id: 'PENDIENTE', label: 'Pendientes' }, { id: 'AL_DIA', label: 'Al día' }, { id: 'TODOS', label: 'Todos' }].map(f => (
            <button key={f.id} onClick={() => setFiltro(f.id)} className={`px-3 py-2 rounded-lg text-[8px] sm:text-[9px] font-black uppercase tracking-widest border transition-all whitespace-nowrap ${filtro === f.id ? 'bg-primary/20 border-primary/40 text-primary' : 'bg-transparent border-white/5 text-slate-600'}`}>
              {f.label}
            </button>
          ))}
        </div>
        <div className="relative w-full sm:ml-auto sm:w-48">
          ...
          <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar..." className="bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-2 text-[10px] text-white outline-none focus:border-primary/30 w-full placeholder:text-slate-700" />
        </div>
      </div>

      {/* Tabla */}
      {loading ? (
        <div className="py-16 text-center text-primary animate-pulse text-[10px] font-black uppercase tracking-widest">Verificando pagos...</div>
      ) : filtrados.length === 0 ? (
        <div className="py-16 text-center space-y-2">
          <span className="material-symbols-outlined text-4xl text-slate-700">{filtro === 'PENDIENTE' ? 'check_circle' : 'payments'}</span>
          <p className="text-slate-600 text-[10px] font-black uppercase tracking-widest">{filtro === 'PENDIENTE' ? '¡Todos al día!' : 'Sin resultados'}</p>
        </div>
      ) : (
        <div className="divide-y divide-white/[0.04]">
          {filtrados.map(a => <FilaAlumno key={a.id} alumno={a} />)}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className={`flex items-center gap-3 sm:gap-4 px-4 sm:px-8 py-4 sm:py-5 hover:bg-white/[0.02] transition-all 
      ...
      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-slate-800 flex items-center justify-center font-black text-primary text-[10px] sm:text-sm border border-white/5 overflow-hidden flex-shrink-0">
        {alumno.foto_url ? <img src={alumno.foto_url} className="w-full h-full object-cover" alt="" /> : `${alumno.primer_nombre?.[0]}${alumno.primer_apellido?.[0]}`}
      </div>
      ...
      <div className="text-right hidden md:block flex-shrink-0">
        ...
        <p className="text-[9px] text-slate-300 font-black">
          {ultimoPago ? format(parseISO(ultimoPago.fecha_pago), "d MMM yyyy", { locale: es }) : 'Sin registros'}
        </p>
      </div>

      <div className="flex flex-col items-end gap-1 flex-shrink-0">
        <span className={`flex items-center gap-1 text-[7px] sm:text-[9px] font-black uppercase px-2 py-1 rounded-md 
                         ${pendiente ? 'text-rose-400 bg-rose-500/10 border border-rose-500/20' : 
                           proximo ? 'text-amber-400 bg-amber-500/10 border border-amber-500/20' : 'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20'}`}>
          <span className="material-symbols-outlined text-[10px] sm:text-xs">
            {pendiente ? 'warning' : proximo ? 'schedule' : 'check_circle'}
          </span>
          {pendiente ? 'Pendiente' : proximo ? 'Cerca' : 'Al día'}
        </span>
        {pendiente && diasMora > 0 && (
          <span className="text-[7px] font-black text-rose-500/80 px-1 leading-none">{diasMora}d mora</span>
        )}
      </div>
    </div>
  );
