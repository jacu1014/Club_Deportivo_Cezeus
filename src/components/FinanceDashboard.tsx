...
const FinanceDashboard = ({ datos = [], userRol, busqueda }: Props) => {
  ...
  const stats = useMemo(() => {
    return datos.reduce((acc, curr) => {
      ...
      if (esIngreso) {
        // NUEVO: acumulamos el total bruto de ingresos pagados
        if (estaPagado) {
          acc.dineroHoy   += valor;
      ...
      return acc;
    }, { dineroHoy: 0, porCobrar: 0, porPagar: 0, totalIngresos: 0 });
  }, [datos]);
  ...
  const dataGraficaSimple = useMemo(() => {
    ...
    const agrupado = datos.reduce((acc: any, d) => {
      ...
      if (CATEGORIAS_FINANZAS.INGRESO.includes(d.categoria)) acc[mesIdx].ingresos += valor;
      else                                                     acc[mesIdx].egresos  += valor;
      return acc;
    }, {});
  ...
  // CORREGIDO: balance real = caja actual - lo que aún debo pagar (sin contar porCobrar)
  const balanceFinal       = stats.dineroHoy - stats.porPagar;
  const tieneDeudaPendiente = stats.porPagar > 0;
  const esPositivo          = balanceFinal >= 0 && !tieneDeudaPendiente;
            ...
            <div className="h-[180px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  >
                    {pieData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} cornerRadius={6} />)}
                  </Pie>
            ...
            <div className="mt-4 space-y-1.5 overflow-y-auto max-h-[120px] pr-2 custom-scrollbar">
              {pieData.slice(0, 4).map((entry, index) => (
                <div key={index} className={`flex justify-between items-center border px-3 py-2 rounded-xl transition-all ${activeIndex === index ? 'bg-white/10 border-white/20' : 'bg-white/[0.02] border-white/5'}`}>
                  ...
                  <span className="text-[10px] font-black text-white">{fmt(entry.value)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">
              {isAdmin ? 'Balance real disponible' : isCoach ? 'Total por recibir' : 'Tu proyección de cuenta'}
            </p>
          </div>
          <div className="flex items-baseline gap-3">
            <h2 className={`text-4xl font-black italic tracking-tighter ${esPositivo ? 'text-white' : 'text-rose-400'}`}>
              {/* CORREGIDO: dineroHoy - porPagar (sin incluir porCobrar) */}
              {isCoach ? fmt(stats.porPagar) : fmt(balanceFinal)}
            </h2>
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${esPositivo ? 'text-emerald-400 border-emerald-400/30 bg-emerald-400/10' : 'text-rose-400 border-rose-400/30 bg-rose-400/10'}`}>
              {isCoach ? (stats.porPagar > 0 ? 'Pendiente' : 'Al día') : (esPositivo ? 'Superávit' : 'Déficit')}
            </span>
          </div>
          {isAdmin && (
            <p className="text-slate-600 text-[9px] font-bold uppercase mt-2 italic">
              Caja actual {fmt(stats.dineroHoy)} — compromisos pendientes {fmt(stats.porPagar)}
            </p>
          )}
        </div>
        ...
        <div className="hidden md:flex flex-col items-center gap-1 px-6 border-x border-white/5">
          ...
          <div className="w-32 h-1.5 bg-white/5 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${esPositivo ? 100 : Math.min(40, (stats.dineroHoy / (stats.dineroHoy + stats.porPagar || 1)) * 100)}%` }}
            />
          </div>
        </div>
